...
# Dashboard IA NetSolutions PRO - Auto-sauvegarde + auto-dépannage

# Créer le script Python du dashboard
cat > ~/dashboard_pro.py << 'PYEOF'
#!/usr/bin/env python3
...
chmod +x ~/dashboard_pro.py
tmux kill-session -t dashboard 2>/dev/null
tmux new-session -d -s dashboard "python dashboard_pro.py"
echo "✅ Dashboard lancé dans la session tmux 'dashboard'."
echo "📌 Pour voir le tableau de bord : tmux attach -t dashboard"
echo "📌 Pour quitter l'affichage : Ctrl+B puis D (le dashboard continue en arrière-plan)"
echo "📌 Pour arrêter le dashboard : tmux kill-session -t dashboard"
